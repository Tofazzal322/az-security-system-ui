...
const MyOrders = () => {
  ...
  useEffect(() => {
    // const url = `https://az-security.onrender.com/products?email=${user.email}&date=${date}`;
    const url = `https://az-security.onrender.com/orders?email=${user.email}`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    const url = `https://az-security.onrender.com/orders/${id}`;
    fetch(url, {
      method: "DELETE",
    })
      .then((res) => res.json())
